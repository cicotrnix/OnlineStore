...
1. **Layout = B (panel de marca dividido).** Panel oscuro `neutral-900` a la izquierda (logo + gauge estático 100% + chips `0 cycles` / `100% health`), columna de formulario sobre superficie blanca a la derecha. En mobile el panel colapsa a una barra superior compacta (logo + tagline una línea) y el form ocupa el ancho.
...
4. **Panel de marca = estático, server-rendered.** Gauge SVG fijo en 100%, **sin GSAP** — auth se mantiene liviana (no se infla el bundle con motion ni Vaul). **Construir el gauge como SVG propio; NO importar `HeroGauge`** (client + GSAP). Fallback visual ya es el estado final; nada que animar.
...
6. **Chip `+10% capacity` = omitido del panel por ahora.** El panel muestra solo constantes spec-aprobadas (`0 cycles`, `100% health`). El `+10%` es dato real, pero su cita formal del fabricante está pendiente (FU-010, ver memoria claims-source); para no propagar un claim sin cita a otra superficie —y dado que hero/StatStrip aún arrastran un `+12%` stale a reconciliar— la capacidad se agrega a auth en la misma pasada en que aterrice la doc del fabricante. Reversible en una línea.

## Arquitectura

### `AuthShell` + `AuthBrandPanel` (nuevos — el shell de dos columnas)

`(auth)/layout.tsx` deja de ser un card centrado y pasa a ser el shell de dos columnas. `AuthBrandPanel` (server, estático) renderiza: logo Pi·Power, un **gauge SVG estático** fijo en 100% (construido como SVG propio — **NO importar `HeroGauge`**, que es client + GSAP y arrastraría GSAP al bundle de auth), headline "Back to 100%", y chips mono `0 cycles` + `100% health` (constantes spec-aprobadas; el chip `+10% capacity` queda fuera por ahora — ver Decisión 6). La columna derecha es el slot (`children`): cada page renderiza su form o su estado ahí. `LocaleSwitch` se reubica en el chrome del shell. En mobile: `AuthBrandPanel` colapsa a barra superior compacta.
...
- **Revoca TODAS las sesiones del usuario** con `prisma.session.deleteMany({ where: { userId } })` directo — **no** `invalidateOtherSessions` (esa mantiene la sesión actual; en el reset no autenticado no hay sesión actual que preservar, hay que matarlas todas). Luego `createDbSession` (sesión fresca) → redirect `/select-org`.
...
- **Reset happy path:** el token crudo solo viaja en el email (noop sin `RESEND_API_KEY`), así que el e2e **crea el `PasswordResetToken` directo** (raw conocido + su hash SHA-256 en DB) y visita `/reset-password/<raw>` → nueva clave → auto-sign-in `/select-org`. Caso `token usado`: re-visitar con `usedAt` seteado → estado "usado". Caso `expirado`: `expiresAt` en el pasado → estado "expirado".
...
- **Migración Prisma nueva** (`PasswordResetToken`) → aplicar en los 3 entornos: `migrate dev` local (necesario para que pasen unit + e2e), el **DB de test del e2e prod-build** (sin la migración el reset rompe en runtime y el e2e falla), y `migrate deploy` en prod (detrás de maintenance) al ir a live. Coordinar con Coolify.
