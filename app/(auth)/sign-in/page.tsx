...
export default async function SignInPage({ searchParams }: Props) {
    ...
    return (
      <div>
        <h1 className="text-2xl font-semibold tracking-tight text-ink-950">
          {t(locale, 'auth.signIn.checkInbox.title')}
        </h1>
        <p className="mt-2 text-sm text-ink-500">{t(locale, 'auth.signIn.checkInbox.body')}</p>
      </div>
    )
  }
  ...
  return (
    <div>
      ...
      <p className="mt-2 text-sm text-ink-500">{t(locale, 'auth.signIn.subtitle')}</p>
      <SignInForm
        locale={locale}
